...
float _lambertDiffuse(vec3 lightDirection, vec3 surfaceNormal){
  ...
  return max(0.3, dot(-lightDirection, surfaceNormal)) * 1.5;
}
...
void totalLight(vec3 modelPosition, vec3 normal, out vec3 totalDiffuse, out vec3 totalSpecular){
  ...
  // なんかloopにするとloopProtectが働いちゃうみたいだから
  // jとかそういうの消してloopやめた
  vec3 lightVector = (uViewMatrix * vec4(uLightingDirection[0], 0.0)).xyz;
  vec3 lightColor = uDirectionalDiffuseColors[0];
  vec3 specularColor = uDirectionalSpecularColors[0];
...
// やっぱりね
// tint処理、普通に255限界で書いてる
// まあsayoさんのコードパクっただけだしそういうのは仕方ないか
// もうtint(1)とか書かずに済むわね
// ていうかtintってテクスチャあるときしか適用されないのね
// それ以外は普通にuMaterialColor・・あれ、頂点色どうするんだろ
// わかった
// aMaterialColorを自前で追加する
// またtintをテクスチャでなくても適用したいならこっちを適宜改造する
// じゃないや違う
// そういうことはvDiffuseColorでやるんじゃない？
void main(void) {
  vec4 col = vVertexColor;
  // col = uMaterialColor;

  if(uEmissive && !isTexture) {
    // Emissiveを使ってなくてistextureでもないなら普通に色を出力するだけ。基本はこれ。
    gl_FragColor = col;
  }else{
    // テクスチャがある場合それを。
    gl_FragColor = isTexture ? texture2D(uSampler, vVertTexCoord) * (uTint / vec4(255, 255, 255, 255)) : col;
    // そしてDiffuseColorは掛け算、SpecularColorは足し算で適用するのね
    //
...
void main() {
  ...
  // ここでマイナス付けるとgl.FRONTのときに描画されるようになる
